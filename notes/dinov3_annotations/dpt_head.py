...
# =============================================================================
# 【中文导读】dpt_head.py —— DPT decoder
# -----------------------------------------------------------------------------
# 输入:ViT 给的 4 个一样大的 (patch_feat, cls_token) 元组
#       例:[(1024, 32, 32), (1024, 32, 32), (1024, 32, 32), (1024, 32, 32)]
# 输出:一张深度图 (B, 1, H, W)
#
# 整体流程(用做菜比喻):
#   STEP 1  Reassemble       人为造金字塔(浅放大、深缩小)+ 融合 CLS
#                            L4 → 128×128, L11 → 64×64, L17 → 32×32, L23 → 16×16
#   STEP 2  Convs            把所有层级通道数对齐成 256
#   STEP 3  FeatureFusion×4  自顶向下逐层融合 + 每步上采 2× (U-Net 解码套路)
#                            从最深最小的 16×16 一路放大融合到 256×256
#   STEP 4  UpConvHead       conv + 上采 + 1×1 conv → 深度图 (B, 1, H, W)
#
# 关键概念:
#   - 上采样: feature map 变大(32→64),用 ConvTranspose2d 或 F.interpolate
#   - 下采样: feature map 变小(32→16),用 stride=2 的 Conv2d
#   - 通道数 C:每个像素携带的信息量,(B, C, H, W) 里的 C
#
# 为什么浅层放大、深层缩小?
#   - 浅层 token 懂细节 → 放大让细节占更多空间
#   - 深层 token 懂语义 → 缩小到瓶颈当"指挥官"
#   - 多尺度组合 > 单尺度,U-Net 经验
#
# 【师兄方向的插入点】
#   方案 A(激进):改 encoder,在 ViT 中间层之间塞 photometric 反馈
#   方案 B(保守):改本文件,每个 FeatureFusion 后出 mid-depth → 算 photo error → 反馈
...
# -----------------------------------------------------------------------------
# 【STEP 4 组件】UpConvHead —— DPT 最后一段:特征 → 深度图
#   输入:  (B, 256, H', W')
#   流程:  3×3 conv → 上采 2× → 3×3 conv → ReLU → 1×1 conv (压成 1 通道)
#   输出:  (B, 1, 2H', 2W')   ← 深度图
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 【STEP 1 主角】ReassembleBlocks —— 把 ViT 输出造成金字塔
#   输入:  list[(patch_feat (B, 1024, h, w), cls_token (B, 1024))]  4 个层级
#   做 3 件事:
#     1) 用 CLS token 调制 patch:   project = Linear([patch ; cls])
#     2) 1×1 conv 改通道数:1024 → [128, 256, 512, 1024]  浅层少通道,深层多通道
#     3) 改分辨率制造金字塔:
#        L4  → ConvTranspose2d stride=4  (放大 4×)  ← 最大,细节
#        L11 → ConvTranspose2d stride=2  (放大 2×)
#        L17 → Identity                  (不变)
#        L23 → Conv2d         stride=2   (缩小 2×)  ← 最小,语义
#   输出:  list[ (B, 128, 4h, 4w), (B, 256, 2h, 2w), (B, 512, h, w), (B, 1024, h/2, w/2) ]
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 【STEP 3 单元】FeatureFusionBlock —— 融合 + 上采 2×(U-Net 解码套路)
#   forward(*inputs):
#     - 1 个输入: 只 conv 处理 + 上采 2×
#     - 2 个输入: x = inputs[0] (主线,来自上一级 fusion),inputs[1] = skip (来自 reassemble)
#                 x = x + ResConvUnit(skip)   ← 残差相加 (skip connection)
#                 ResConvUnit2(x) → 上采 2× → 1×1 project
#   作用: 把更深的主线 + 更浅的 skip 融合,逐步放大,最终还原到原图分辨率
# -----------------------------------------------------------------------------
...
# =============================================================================
# 【主类】DPTHead —— 串起 STEP 1~4 的总装
#   forward(inputs):
#     forward_features() 走 STEP 1~3,得到 (B, 256, H', W')
#     conv_depth (UpConvHead) 走 STEP 4,得到 (B, 1, H, W) 深度图
#
#   forward_features 内部:
#     x = self.reassemble_blocks(x)               # STEP 1: 造金字塔
#     x = [self.convs[i](f) for i, f in ...]      # STEP 2: 通道对齐到 256
#     out = self.fusion_blocks[0](x[-1])          # STEP 3: 从最深 (L23) 开始
#     for i in 1..3:
#         out = self.fusion_blocks[i](out, x[-(i+1)])  # 自顶向下 + skip
# =============================================================================
class DPTHead(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv_depth = UpConvHead(self.channels, self.n_output_channels, self.n_hidden_channels)
